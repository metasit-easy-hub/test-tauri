  ...
  document.getElementById("btn-delete").addEventListener("click", async () => {
    ...
    if (confirm("คุณต้องการลบไฟล์ที่เลือกใช่หรือไม่?")) {
      for await (const entry of problemFolder.values()) {
        if (entry.kind === "file" && entry.name.toLowerCase().endsWith(".webm") && storeSelectMove.includes(entry.name)) {
    ...
    try {
      ...
      for (let i = 0; i < recordedVideo.length; i++) {
        ...
        try {
          ...
          if (name_file.length == 3 && isOnlyDigits(name_file[0]) && name_file[1].substring(0, 3) === "INV") {
    ...
    //  file-problem
      ...
      for await (const entry of problemFolder.values()) {
        if (entry.kind == "file") {
          ...
          dataText += entry.name + "\n";
          if (entry.name.toLowerCase().endsWith(".webm")) {
            document.getElementById("deleteList").innerHTML += `<div class="px-2 a-click" onclick='selectMove(event)' id='${
              entry.name
            }'>${entry.name}</div>`;
          }
        }
